...
if (window.isMultiViewInjected) {
    console.log("Multi-View script already injected. Listening for new init message.");
} else {
    window.isMultiViewInjected = true;
    console.log("Multi-View content script loaded for the first time.");
}
...
function initMultiView(urls) {
    ...
    Object.assign(container.style, {
        ...
        zIndex: '2147483647', backgroundColor: '#333', display: 'flex', gap: '5px',
        padding: '40px 5px 5px 5px', boxSizing: 'border-box'
    });
    ...
    Object.assign(closeButton.style, {
        padding: '5px 10px', cursor: 'pointer', border: '1px solid #555',
        backgroundColor: '#444', color: 'white', borderRadius: '4px'
    });
    ...
    closeButton.onclick = exitMultiView;
    ...
    const handleEscKey = (event) => { if (event.key === 'Escape') { exitMultiView(); } };
...
function createIframe(label, src) {
    ...
    Object.assign(labelDiv.style, {
        position: 'absolute', top: '5px', left: '5px', padding: '3px 6px',
        backgroundColor: 'rgba(0, 0, 0, 0.7)', color: 'white',
        fontSize: '12px', borderRadius: '3px', zIndex: '1'
    });
