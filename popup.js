...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Get all DOM elements and store them.
    const DOMElements = {};
    ...
    function showStatus(message, isError = false, targetDivId = 'status') {
        ...
        } else if (targetDivId === 'htmlCheckerResult') {
            targetDiv.classList.add('success'); // Use success style for results
        } else {
            // This case is for overlay status messages, which we style as 'info'
            targetDiv.classList.add('info');
        }

        if (targetDivId === 'status') { // Results for HTML checker should persist and not timeout
            statusClearTimer = setTimeout(() => {
                if (DOMElements.status && DOMElements.status.textContent === displayMessage) {
    ...
    function setControlsDisabled(group, busy) {
        ...
        if (group === 'urlTools') {
            [DOMElements.addParamButton, DOMElements.paramNameSelect, DOMElements.paramNameCustom, DOMElements.paramValue, DOMElements.clearParamsButton, DOMElements.envSelect]
        ...
        // Ruler button is a top-level tool, disable it if any async page action is busy
    ...
    if (DOMElements.addParamButton) DOMElements.addParamButton.addEventListener('click', async () => {
        ...
        try {
            ...
            const values = paramValueToUse.split(',').map(s => s.trim()).filter(Boolean);
            ...
            for (let i = 0; i < Math.min(names.length, values.length); i++) {
                urlObject.searchParams.set(names[i], values[i]);
                await saveSuggestion(names[i], values[i]);
                paramsAddedCount++;
            }
            if (paramsAddedCount > 0) {
                await chrome.tabs.update(tab.id, { url: urlObject.toString() });
                showStatus(`Added ${paramsAddedCount} parameter(s).`, false);
                if (DOMElements.paramNameCustom) DOMElements.paramNameCustom.value = '';
                if (DOMElements.paramValue) DOMElements.paramValue.value = '';
                if (DOMElements.paramNameSelect) DOMElements.paramNameSelect.value = '';
    ...
    if (DOMElements.clearParamsButton) DOMElements.clearParamsButton.addEventListener('click', async () => {
        ...
        try {
            ...
            urlObject.search = '';
            await chrome.tabs.update(tab.id, { url: urlObject.toString() });
            showStatus('Parameters cleared.', false);
        } catch (error) { console.error("Clear params error:", error.message); }
        finally { setControlsDisabled('urlTools', false); }
    });

    if (DOMElements.envSelect) {
        DOMElements.envSelect.addEventListener('change', async (e) => {
    ...
    // --- HTML TAG CHECKER LOGIC ---
    if (DOMElements.checkHtmlButton) {
        DOMElements.checkHtmlButton.addEventListener('click', () => {
            ...
            const tableTags = (htmlInput.match(/<table/gi) || []).length;
            const trTags = (htmlInput.match(/<tr/gi) || []).length;
            const tdTags = (htmlInput.match(/<td/gi) || []).length;
            const resultMessage = `Found: ${tableTags} <table>, ${trTags} <tr>, ${tdTags} <td>.`;
            showStatus(resultMessage, false, 'htmlCheckerResult');
        });
    }
    ...
    async function sendMessageToContentScript(action, data = {}, source = 'toolkit-popup-overlay') {
        ...
        try {
            ...
            else if (response && response.status) showStatus("Tool: " + response.status, false, 'status');
    ...
    if (DOMElements.rulerModeButton) {
        DOMElements.rulerModeButton.addEventListener('click', () => {
            sendMessageToContentScript('toggleRulerMode', {}, 'toolkit-popup-ruler');
            window.close();
        });
    }
    
    function showOverlayControls() {
        if (DOMElements.overlayControls) DOMElements.overlayControls.style.display = currentOverlaySettings.imageData ? 'block' : 'none';
        if (DOMElements.overlayIconToolbar) DOMElements.overlayIconToolbar.style.display = currentOverlaySettings.imageData ? 'flex' : 'none';
    }
    ...
    function updateOverlayControlsUI() {
        ...
        DOMElements.overlayOpacity.value = Math.round(100*currentOverlaySettings.opacity); DOMElements.overlayOpacityValue.textContent = Math.round(100*currentOverlaySettings.opacity);
        DOMElements.overlayScaleInput.value = currentOverlaySettings.scale.toFixed(2);
        DOMElements.iconToggleVisibility.innerHTML = currentOverlaySettings.visible?"VIS":'<span style="color:#999;text-decoration:line-through;">VIS</span>'; DOMElements.iconToggleVisibility.title = currentOverlaySettings.visible?"Hide Overlay":"Show Overlay";
        ...
        DOMElements.iconToggleLock.innerHTML = currentOverlaySettings.locked?"LCKD":"LOCK"; DOMElements.iconToggleLock.title = currentOverlaySettings.locked?"Unlock Controls":"Lock Controls";
        ...
        const isBusy = DOMElements.loadingSpinnerOverlay?.classList.contains("active") || false;
        setControlsDisabled("overlay", isBusy);
    }
            ...
            if (file && file.type.startsWith("image/")) {
                ...
                reader.onload = async (e) => {
                    currentOverlaySettings = {
                        imageData: e.target.result, visible: true, scale: 1,
                        x: 0, y: 0, opacity: 1, locked: false
                    };
                    try {
                        await chrome.storage.local.set({ [STORAGE_KEYS.overlayImage]: currentOverlaySettings.imageData, [STORAGE_KEYS.overlaySettings]: { x: 0, y: 0, opacity: 1, scale: 1, visible: true, locked: false } });
                        updateOverlayControlsUI(); showOverlayControls();
                        sendMessageToContentScript("create", { imageData: currentOverlaySettings.imageData, settings: currentOverlaySettings });
                    } catch (storageError) { showStatus("Failed to save overlay image.", true); }
                };
                reader.onerror = () => { showStatus("Error reading image file.", true); };
                reader.readAsDataURL(file);
            } else if (file) { showStatus("Please select a valid image file.", true); DOMElements.imageUpload.value = ""; }
        });
    }

    function handleOverlaySettingChange(key, value) {
        if (currentOverlaySettings.locked && !["visible", "locked"].includes(key)) {
            showStatus("Controls are locked! Unlock to make changes.", true); return;
        }
        ...
        sendMessageToContentScript("update", { settings: settingsToSave });
    ...
    async function loadInitialOverlayState() {
        ...
        try {
            ...
            if (result[STORAGE_KEYS.overlayImage] && result[STORAGE_KEYS.overlaySettings]) {
                currentOverlaySettings.imageData = result[STORAGE_KEYS.overlayImage];
                const s = result[STORAGE_KEYS.overlaySettings];
                ...
                currentOverlaySettings.scale = s.scale !== undefined ? parseFloat(s.scale) : 1.0;
                currentOverlaySettings.locked = s.locked !== undefined ? s.locked : false;
            }
        } catch (e) { console.error("Error loading overlay state:", e), showStatus(e.message || "Error loading overlay state.", true) }
    }
    ...
    async function initializeApp() {
        ...
        } catch (e) { console.warn("initializeApp: Tab not valid for some tools. Error:", e.message); }
